...
@click.option(
    "--max-length", default=2000, help="Maximum length for each split section."
)
@click.option(
    "--use-chatgpt/--no-chatgpt", default=True, help="Use ChatGPT for formatting text."
)
...
@click.argument("input_folder", type=click.Path(exists=True, file_okay=False))
@click.option(
    "--max-words", default=2000, help="Maximum number of words per combined file."
)
    ...
    for file_path in sorted(input_path.glob("*.txt")):
        with open(file_path, "r", encoding="utf-8") as file:
            ...
            if current_word_count + word_count > max_words and combined_content:
                output_filename = (
                    f"combined_{file_count}_words_{current_word_count}.txt"
                )
